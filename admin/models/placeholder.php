...
class ComponentbuilderModelPlaceholder extends JModelAdmin
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_componentbuilder.placeholder';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'placeholder', $prefix = 'ComponentbuilderTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		if ($id != 0 && (!$user->authorise('placeholder.edit.state', 'com_componentbuilder.placeholder.' . (int) $id))
			|| ($id == 0 && !$user->authorise('placeholder.edit.state', 'com_componentbuilder')))
		{
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('placeholder.delete', 'com_componentbuilder.placeholder.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('placeholder.edit.state', 'com_componentbuilder.placeholder.' . (int) $recordId);
		...
		return $user->authorise('placeholder.edit.state', 'com_componentbuilder');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('placeholder.edit', 'com_componentbuilder.placeholder.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('placeholder.edit',  'com_componentbuilder');
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_componentbuilder.edit.placeholder.data', array());
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		elseif (isset($values['published']) && !$this->canDo->get('placeholder.edit.state'))
		{
				$values['published'] = 0;
		}
		...
		while (!empty($pks))
		{
			...
			if (!$this->user->authorise('placeholder.edit', $contexts[$pk]))
			{
				// Not fatal error
	...
	protected function batchMove($values, $pks, $contexts)
	{
		...
		if (isset($values['published']) && !$this->canDo->get('placeholder.edit.state'))
		{
